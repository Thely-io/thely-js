...
var compare = (currentData, newData) =>
...
var SWRConfig = (props) => {
	...
	const extendedConfig = import_react.useMemo(
...
var useSWRHandler = (_key, fetcher, config) => {
	...
	const cached = shim.useSyncExternalStore(
	...
	const isLoading = isUndefined(cached.isLoading)
		? defaultValidatingState
		: cached.isLoading;
	const revalidate = import_react2.useCallback(
...
var useQuery = (key, config) => {
	const {
		...
		error,
		isValidating,
	} = useSWR(key ?? null, config);
	return { data: data?.data, mutate: mutate2, isLoading, error, isValidating };
};

// lib/hooks/useInit.ts
var useInit = (accountId) => {
	const { data, error } = useQuery(`app/init/${accountId}`);
	return {
		data,
		error,
	};
