...
export const loginUser = async (data: z.infer<typeof loginUserSchema>) =>
  await axiosApiInstance.post("customer/login", data);

export const registerUser = async (data: z.infer<typeof registerUserSchema>) =>
  await axiosApiInstance.post("customer/signup", data);

export const createRepack = async (id: string) =>
  await axiosApiInstance.post("/v1/tasks/repack", { id });
...
export const updateItem = async (data: z.infer<typeof updateItemSchema>) =>
  await axiosApiInstance.patch("customer/item", data);
...
export const selectRate = ({ id, rate_id }: { id: string; rate_id: string }) =>
  axiosApiInstance.patch(`customer/shipment`, {
...
) => await axiosApiInstance.post("customer/address", data);
